...
- **Ingress**	:  Manages HTTP/HTTPS access into Services (routing, SSL, etc.).
...
4. Storage Concepts
Concept	Meaning
Volume	Storage that a Pod can use.
Persistent Volume (PV)	Pre-provisioned storage in the cluster.
Persistent Volume Claim (PVC)	A Pod’s request for storage.
StorageClass	Defines types of storage (like SSD or HDD).
5. Configuration & Secrets Management
Concept	Meaning
ConfigMap	Stores non-sensitive configuration data.
Secret	Stores sensitive data like passwords, API keys.
Environment Variables	Dynamic app configuration via environment variables inside Pods.
...
Affinity/Anti-Affinity	Scheduling rules: Pods that prefer (or avoid) certain Nodes/Pods.
Taints & Tolerations	Forces Pods to run only on specific Nodes or avoid others.
PodDisruptionBudget	Limits voluntary disruptions to ensure app availability.
7. Security Concepts
Concept	Meaning
RBAC (Role-Based Access Control)	Controls what users or apps can access/manage inside Kubernetes.
Role / ClusterRole	Defines permissions (Role = namespace level, ClusterRole = cluster level).
RoleBinding / ClusterRoleBinding	Grants permissions to users/groups/service accounts.
ServiceAccount	Identity for processes running inside Pods.
Network Policy	Controls Pod communication rules.
PodSecurityContext	Defines security settings inside a Pod (user, group, file permissions).
